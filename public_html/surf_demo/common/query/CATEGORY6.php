<?php
	$scn = $serverInfo['SCN_CATEGORY6'][0]['value'];
	$orderBy = "";
	$orderNm = "";
	...
	if ($srchParam->getKwd() != "") {			
		...
		// 로그 생성 		
		if ($srchParam->getCategory() == "CATEGORY6") {
		...
		// 조건 쿼리 추가 
		$query = makeQuery("category_cd", "CATEGORY6", "", $query, "and");
		...
		dcSubmitQuery($scn, $query, $logInfo, $hilightTxt, 
					$orderBy, $pageNum, $srchParam->getPageSize(), true, $rsbCATEGORY6, 1, 1);
	}
