...
$thumb_domain			= "http://"._DOMAIN_FILE."/frontier/list_img/";
...
//echo Module::$param[0]."___".Module::$param[3];;
...
for($i=0;$i<sizeof($talkData);$i++) {
	...
	if($frontierData['frno'] > "877"){   
		...
		$top_thumbimg					= $thumb_domain.$topimgDir."/".$frontierData['titleimg'];
	}else{
...
$fb_start_date = $frontierData['start_date'];
$fb_start_date = substr($fb_start_date,0,4)."-".substr($fb_start_date,5,2)."-".number_format(substr($fb_start_date,8,2));

$fb_end_date = $frontierData['end_date'];
$fb_end_date = substr($fb_end_date,0,4)."-".substr($fb_end_date,5,2)."-".number_format(substr($fb_end_date,8,2));
...
$nowPageUrl = "http://"._DOMAIN.$_SERVER['REQUEST_URI'];

// Layer Define <invisble>
...
$TPL->setValue(array(
	...
	"topimg"=>$topimg,
	"top_thumbimg"=>$top_thumbimg,
	"cate"=>$cate,
	"frproduct"=>$frproduct,
	"jstart_date"=>$jstart_date,
	"jend_date"=>$jend_date,
	"fb_start_date"=>$fb_start_date,
