...
{	
	...
	case "sns.proc" :
		...
		// 기본정보 가입처리
		$acArr = array();
		$acArr[] = $_POST['userid'];// $userid; // 아이디 
		$acArr[] = $_POST['username']; // 이름 
		$acArr[] = $_POST['passwd']; //$userid; // 비밀번호
		...
		$res = array();
		$res[] = $DB->call("p_account_sns_20120913_ins", $acArr);
		...
		// SNS회원정보-> 모든 회원은 레뷰회원을 기준으로함(SNS계정은 연결하기로이용됨)	
//		$acArr = array();
//		$acArr[] = $type; // 회원타입
//		$acArr[] = $res[0]['ouserno']; // 회원번호
//		$acArr[] = $userid; // 트위터아이디
...
//		$res[] = $DB->call("p_account_sns_ins", $acArr);
		...
		if(Module::result($res, "err", "-1") == false) {
			...
			$CLASS_LOGIN->addSession($frontierInfo2);			
			if($type == "F"){
				$_SESSION["fb_nickname"] = $name;
			}else{
				$_SESSION['twit_nickname'] = $name; // 트위터이름
			}
